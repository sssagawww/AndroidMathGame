...
public class Controller extends Table {
    ...
    public Controller(Skin skin) {
        ...
        float size = Gdx.graphics.getHeight() / 9f;
        Table table = new Table();
        table.add(menuImg).width(size).height(size).align(Align.topLeft).row();
        table.add(settingsImg).width(size).height(size).align(Align.topRight);
        this.add(table).top();
        this.add(inventory).expand().align(Align.center);
        this.add(inventImg).width(size).height(size).align(Align.topRight).padRight(15f);
