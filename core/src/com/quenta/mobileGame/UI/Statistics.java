...
import static com.quenta.mobileGame.MyGdxGame.gameTime;
...
public class Statistics extends Table {
    ...
    public Statistics(Skin skin) {
        ...
        topTable.add(exitImage).align(Align.left).width(Gdx.graphics.getHeight()/12f).height(Gdx.graphics.getHeight()/12f).padTop(10f);
        uiTable.add(topTable).align(Align.top).width(Gdx.graphics.getWidth() / 2f).height(Gdx.graphics.getHeight()/12f).row();
