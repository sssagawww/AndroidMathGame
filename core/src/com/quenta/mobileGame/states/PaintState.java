...
import com.quenta.mobileGame.UI.PaintMenu;
...
import com.quenta.mobileGame.paint.DistanceCalc;
import com.quenta.mobileGame.paint.Figures.FiguresDatabase;
import com.quenta.mobileGame.paint.PixelPoint;
...
public class PaintState extends GameState implements InputProcessor {
    ...
    private final ArrayList<PixelPoint> points;
    private final ArrayList<Integer> skippedPoints = new ArrayList<>();
    ...
    private final Array<Vector2> arr = new Array<>();
    ...
    public void render() {
        ...
        for (int i = 0; i < getFigureHints().size(); i++) {
            shapeRenderer.rect(getFigureHints().get(i).getX()*(Gdx.graphics.getWidth()/1800f), getFigureHints().get(i).getY()*(Gdx.graphics.getWidth()/1800f), rectWidth, rectHeight);
        }
        shapeRenderer.end();
        sb.end();
    ...
    public void initUI() {
        ...
        if (online) {
            ...
            onlineRoot.add(scoreTable).align(Align.topLeft).padTop(Gdx.graphics.getHeight() / 9f).padBottom(Gdx.graphics.getHeight() / 11.8f).expand().row();
            readyRoot.add(readyLabel).width(Gdx.graphics.getWidth()).height(Gdx.graphics.getHeight() / 4f).expand().center();
        }

        Table table = new Table();
        table.add(initBtn()).expand().align(Align.bottomRight).width(Gdx.graphics.getHeight() / 9f).height(Gdx.graphics.getHeight() / 9f).pad(15f);
        table.add(paintMenu).expand().align(Align.right);
        root.add(menuImg).align(Align.topLeft).width(Gdx.graphics.getHeight() / 9f).height(Gdx.graphics.getHeight() / 9f).padTop(20f).expand();
    ...
    private Table initBtn() {
        ...
        table.add(btn).width(Gdx.graphics.getHeight() / 9f).height(Gdx.graphics.getHeight() / 9f);
        return table;
    }
