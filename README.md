...
Quenta: Путь ученика - это развивающая игра с пиксельной графикой, рассчитанная на детей дошкольного возраста и уровня начальных классов. В ней игроку предстоит пройти через испытания, чтобы собрать артефакты и с их помощью победить зло. Различные игровые механики тренируют знания математики, реакцию и координацию движений рук. Также игрок может проходить сюжет, исследовать мир, получать достижения.

## Содержание
1. [Как играть по сети](#onlineTitle)
2. [Краткое описание работы](#howTitle)
3. [Классы](#classesTitle)
4. [Описание работы алгоритма сравнения нарисованных фигур](#algTitle)
5. [Предложения по изменению](#fixesTitle)
6. [Новые возможности и исправления](#newsTitle)
...
* Если IP-адрес верен, он останется в поле ввода. В противном случае, он будет очищен, и вам необходимо будет ввести его снова и подтвердить галочкой. Статус подключения отображется сверху.

2. Выбор режима:

* После успешного подключения к серверу выберите желаемый игровой режим.
* Далее создайте новую комнату для игры или же подключитесь к другой, введя ID комнаты.
...
* Проверка IP-адреса:
    ...
    * Иногда запрос на сервер может не успеть отправиться, и даже при правильном IP-адресе игра может выдать ошибку. В этом случае, рекомендуется перезайти в игру или в меню сетевой игры (например, откройте главное меню игры и выйдите обратно).
* Сохранение данных:
    ...
    * IP сервера также сохранится при успешном подключении в первый раз.
...
## <a id="howTitle">Краткое описание работы</a>
...
## <a id="algTitle">Описание работы алгоритма сравнения нарисованных фигур</a>
...
## <a id="fixesTitle">Предложения по изменению</a>
...
- Многопользовательский режим находится в процессе доработки.

## <a id="newsTitle">Новые возможности и исправления! 🐇⚔️ </a>
...
- 📄 Обновлена система сохранения: теперь можно сохранить прогресс в любой локации и начать новую игру сразу после пройденной.
- 🐾 Исправлены визуальные баги с зверями на карте и предметами в инвентаре.
- 📱 Увеличен пользовательский интерфейс и обновлено меню настроек звуков.
- 🎮 Добавлен динамический джойстик для более удобного управления.
- 📐 Доработана механика с математикой.
- 🧩 Обновлена локация Лабиринт и коллизия в Лесу.
